...
public class AnalyticsService {
    ...
    public SummaryResponse getSummary() {
        ...
        List<Task> teamTasks =
                taskRepository.findByAssignedById(manager.getId());
        ...
        Map<Long, List<Task>> tasksByEmployeeMap =
                teamTasks.stream()
                        .filter(task ->
                ...
                team.stream()
                        .map(employee -> {
                            ...
                            List<Task> employeeTasks =
                                    tasksByEmployeeMap.getOrDefault(
                                            employee.getId(),
                                            Collections.emptyList()
                                    );

                            int active =
                                    (int) employeeTasks.stream()
                                            .filter(task ->
                                                    task.getStatus()
                                                            != TaskStatus.DONE
                                            )
        ...
        Map<String, Integer> leaveReasonMap =
                new HashMap<>();
